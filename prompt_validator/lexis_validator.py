"""LEXIS-specific prompt system validator.

Validates Legal/Compliance Decision System prompts with both generic
prompt quality checks AND domain-specific legal integrity checks.

Usage:
    python -m prompt_validator.lexis_validator prompts/lexis_compliance.txt
"""
...
from prompt_validator.lexis_rules import ALL_LEXIS_RULES


class LexisValidator:
    def __init__(self):
        self.generic_rules = GENERIC_RULES
        self.domain_rules = ALL_LEXIS_RULES
    ...
    def validate(self, prompt: str) -> dict:
        ...
        if Category.ACCURACY in cats:
            domain_suggestions.append("Fix legal formula definitions and statute references.")
            domain_suggestions.append("Verify all penalty thresholds and jurisdiction rules are consistent.")
        if Category.STRUCTURE in cats:
            domain_suggestions.append("Ensure all required legal sections are present and numbered.")
        has_decision_issue = any("decision" in i.message.lower() or "compliance" in i.message.lower() for i in domain_result.issues)
        if has_decision_issue:
            domain_suggestions.append("Review compliance decision tree — every state needs clear transition rules.")
...
def print_report(results: dict):
    ...
    print("=" * 70)
    print("  LEXIS LEGAL COMPLIANCE SYSTEM — FULL VALIDATION REPORT")
    print("=" * 70)
    ...
    print()
    print("-" * 70)
    print("  PART 2: DOMAIN-SPECIFIC (LEGAL COMPLIANCE INTEGRITY)")
    ...
    if score >= 85: verdict = "PASS — System is well-structured and ready for regulatory deployment."
    ...
    else: verdict = "FAIL — Critical problems detected; not ready for compliance use."
    print(f"  {verdict}")
...
def main():
    parser = argparse.ArgumentParser(description="Validate a LEXIS legal compliance prompt system.")
    parser.add_argument("file", help="Path to the LEXIS prompt file.")
