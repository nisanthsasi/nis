"""SENTINEL-specific prompt system validator.

Validates Cybersecurity/Threat Detection System prompts with both generic
prompt quality checks AND domain-specific security integrity checks.

Usage:
    python -m prompt_validator.sentinel_validator prompts/sentinel_security.txt
"""
...
from prompt_validator.sentinel_rules import ALL_SENTINEL_RULES


class SentinelValidator:
    def __init__(self):
        self.generic_rules = GENERIC_RULES
        self.domain_rules = ALL_SENTINEL_RULES
    ...
    def validate(self, prompt: str) -> dict:
        ...
        if Category.ACCURACY in cats:
            domain_suggestions.append("Fix severity scoring formulas and MITRE ATT&CK mappings.")
            domain_suggestions.append("Verify all IoC classifications and correlation rules are consistent.")
        if Category.STRUCTURE in cats:
            domain_suggestions.append("Ensure all required security sections are present and numbered.")
        has_alert_issue = any("alert" in i.message.lower() for i in domain_result.issues)
        if has_alert_issue:
            domain_suggestions.append("Review alert FSM — every state needs entry/exit paths and escalated alerts need resolution.")
...
def print_report(results: dict):
    ...
    print("=" * 70)
    print("  SENTINEL THREAT DETECTION SYSTEM — FULL VALIDATION REPORT")
    print("=" * 70)
    ...
    print()
    print("-" * 70)
    print("  PART 2: DOMAIN-SPECIFIC (THREAT DETECTION INTEGRITY)")
    ...
    if score >= 85: verdict = "PASS — System is well-structured and ready for SOC deployment."
    ...
    else: verdict = "FAIL — Critical problems detected; not ready for security operations."
    print(f"  {verdict}")
...
def main():
    parser = argparse.ArgumentParser(description="Validate a SENTINEL threat detection prompt system.")
    parser.add_argument("file", help="Path to the SENTINEL prompt file.")
