"""QUANTA-specific prompt system validator.

Validates Scientific Computing/Research System prompts with both generic
prompt quality checks AND domain-specific scientific rigor checks.

Usage:
    python -m prompt_validator.quanta_validator prompts/quanta_research.txt
"""
...
from prompt_validator.quanta_rules import ALL_QUANTA_RULES


class QuantaValidator:
    def __init__(self):
        self.generic_rules = GENERIC_RULES
        self.domain_rules = ALL_QUANTA_RULES
    ...
    def validate(self, prompt: str) -> dict:
        ...
        if Category.ACCURACY in cats:
            domain_suggestions.append("Fix scientific formula definitions and ensure all variables are defined.")
            domain_suggestions.append("Verify error bounds and convergence criteria are specified.")
        if Category.STRUCTURE in cats:
            domain_suggestions.append("Ensure all required methodology sections are present and numbered.")
        has_repro_issue = any("reproducibility" in i.message.lower() or "seed" in i.message.lower() for i in domain_result.issues)
        if has_repro_issue:
            domain_suggestions.append("Specify random seeds and version locks for full reproducibility.")
...
def print_report(results: dict):
    ...
    print("=" * 70)
    print("  QUANTA SCIENTIFIC COMPUTING SYSTEM — FULL VALIDATION REPORT")
    print("=" * 70)
    ...
    print()
    print("-" * 70)
    print("  PART 2: DOMAIN-SPECIFIC (SCIENTIFIC COMPUTING INTEGRITY)")
    ...
    if score >= 85: verdict = "PASS — System is well-structured and ready for research deployment."
    elif score >= 70: verdict = "CONDITIONAL PASS — Minor issues found; review methodology."
    elif score >= 50: verdict = "NEEDS WORK — Significant issues that should be addressed."
    else: verdict = "FAIL — Critical problems detected; not ready for publication."
    print(f"  {verdict}")
...
def main():
    parser = argparse.ArgumentParser(description="Validate a QUANTA scientific computing prompt system.")
    parser.add_argument("file", help="Path to the QUANTA prompt file.")
